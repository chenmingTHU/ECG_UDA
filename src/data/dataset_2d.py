...
def load_beat_with_rr(path, data_dir, n, fs, fixed_len, lead=1):

    assert n >= 1, "The number of adjacent beat must be larger than 1."
    ...
    low_bound_pre = peaks[index - n - 1] + left_points
    high_bound_pre = peak + right_points
    beat1 = data_dir[record_id + '.mat']['signal'][0, low_bound_pre: high_bound_pre + 1]
    l1 = len(beat1)
    beat1 = ss.resample(beat1, fixed_len * l1).astype(np.float32)
    beat1 = beat1.reshape((fixed_len, l1))
    beat1 = np.mean(beat1, axis=1, keepdims=True)
    low_bound_post = peaks[index - 1] + left_points
    high_bound_post = peaks[index + n] + right_points
    beat2 = data_dir[record_id + '.mat']['signal'][0, low_bound_post: high_bound_post + 1]
    l2 = len(beat2)
    beat2 = ss.resample(beat2, fixed_len * l2).astype(np.float32)
    beat2 = beat2.reshape((fixed_len, l2))
    beat2 = np.mean(beat2, axis=1, keepdims=True)
    dual_beat = np.matmul(beat1, beat2.T)
    max_val = np.max(dual_beat)
    min_val = np.min(dual_beat)
    dual_beat = dual_beat / (max_val - min_val)

    dual_beat = np.expand_dims(dual_beat, axis=0)

    return dual_beat, cls


def augmentation_transform_with_rr(path, data_dir, n, fs, fixed_len, lead=1):
    ...
    left_points = int((0.14 + 0.01 * np.random.rand(1)) * fs)
    right_points = int((0.28 + 0.01 * np.random.rand(1)) * fs)
...
class MULTI_ECG_TRAIN_DATASET(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dual_beat, cls = transform(file_path, self.data,
                                   n=self.n, fs=self.fs,
...
class MULTI_ECG_EVAL_DATASET(Dataset):
    ...
    def __init__(self, dataset_name, data_loader, data_dict, test_records, beat_num=1, fixed_len=200, lead=1):
    ...
    def __getitem__(self, index):
        ...
        dual_beat, cls = self.loader(file_path, self.data,
                                     n=self.n, fs=self.fs,
                                     fixed_len=self.fixed_len,
                                     lead=self.lead)
        return dual_beat, cls
...
class UDA_DATASET(Dataset):
    ...
    def __init__(self,
                 ...
                 fixed_len=200, lead=1, use_dbscan=False):
    ...
    def __getitem__(self, index):
        ...
        pack = transform(file_path_s, self.data_s, n=self.n, fs=self.fs_s, fixed_len=self.fixed_len, lead=self.lead)
        dual_beat_s = pack[0]
        ...
        pack_u = transform(file_path_t, self.data_t, n=self.n, fs=self.fs_t, fixed_len=self.fixed_len, lead=self.lead)
        dual_beat_t = pack_u[0]

        cls_s = pack[1]
        cls_t = pack_u[1]

        return (dual_beat_s, cls_s,
                dual_beat_t, cls_t)
