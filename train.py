...
def obtain_pesudo_labels(net, loaded_models, t_batch, thrs, use_thr=True):
        ...
        for epoch in range(max(pre_train_epochs, check_epoch), 2 * pre_train_epochs):
            ...
            for idb, data_batch in enumerate(dataloader):
                ...
                if cfg.SETTING.CLoss:
                    loss += loss_cs * w_cs
                if cfg.SETTING.INTER_LOSS:
                    loss += loss_inter * w_inter
                if cfg.SETTING.INTRA_LOSS:
                ...
                running_lr = optimizer_re.state_dict()['param_groups'][0]['lr']
                torch.cuda.empty_cache()

                if idb % 10 == 9:
                    print("[{}, {}] cls loss: {:.4f}, "
                          "lr: {:.5f}".format(epoch, idb, loss_cls, running_lr))
                    torch.save({'model_state_dict': net.state_dict()},
                               osp.join(save_path, '{}.pkl'.format(epoch)))
